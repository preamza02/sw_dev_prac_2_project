...
export default function BookedHotelCard({ booking }: BookedHotelCardProps) {
    ...
    const [startDateForInput, setStartDateForInput] = useState(new Date(booking.bookingDate).toISOString().split('T')[0]);
    const endDateString = new Date(booking.checkoutDate).toLocaleDateString('en-GB', {
        day: 'numeric',
        month: 'short',
        year: 'numeric'
    });
    const [endDateForInput, setEndDateForInput] = useState(new Date(booking.checkoutDate).toISOString().split('T')[0]);
        ...
        <div className='border p-4 w-[280px] rounded-lg shadow-md border-gray-300' onClick={() => setIsEditing(!isEditing)}>
            ...
            {isEditing && (
                ...
                <div onClick={(e) => e.stopPropagation()}>
                    ...
                    <p>Check-in</p>
                    <input type='date' className='w-full mb-4' id='check-in' value={startDateForInput} onChange={(e) => setStartDateForInput(e.target.value)} />
                    <p>Check-out</p>
                    <input type='date' className='w-full mb-4' id='check-out' value={endDateForInput} onChange={(e) => setEndDateForInput(e.target.value)} />
